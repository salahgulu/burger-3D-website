...
export default function ScrollMenuExperience() {
  ...
  useEffect(() => {
    const loadedImages = [];
    const maxFramesToTry = 600; // The video length dictates this; we'll try up to 600
    let loaded = 0;
    let failed = 0;
    let consecutiveFails = 0;
    let totalFramesFound = 0;

    // Load sequentially to know when we've hit the end
    const loadNext = (index) => {
      if (index > maxFramesToTry || consecutiveFails > 5) {
      ...
      img.src = `/frames-menu/frame_${paddedIndex}.jpg`;

      img.onload = () => {
        consecutiveFails = 0;
        loaded++;
        loadedImages[index - 1] = img;
        totalFramesFound = Math.max(totalFramesFound, index);
        
        // Progress is an approximation assuming ~300 frames
        setLoadProgress(Math.min(100, Math.round((loaded / 300) * 100)));
  ...
  return (
    <section className="relative h-[400vh]" ref={containerRef}>
      <div className="sticky top-0 h-screen w-full overflow-hidden">
          ...
          <div className="absolute inset-0 z-30 flex flex-col items-center justify-center bg-[var(--color-bg-deep)]">
            <div className="relative w-20 h-20 mb-6">
              <svg className="w-20 h-20 -rotate-90" viewBox="0 0 80 80">
                ...
                />
                <defs>
                  <linearGradient id="gradient-menu" x1="0" y1="0" x2="1" y2="1">
            ...
            <p className="text-neutral-400 text-sm">Loading Menu 3D Experience...</p>
          </div>
        )}
        ...
        >
          <span className="text-red-500 text-sm font-semibold tracking-[0.3em] uppercase mb-4">
            3D Menu Showcase
          </span>
          <h2
          ...
          >
            Dive into the
            <br />
            <span className="gradient-text-warm italic">Details</span>
          </h2>
        </motion.div>
        ...
        >
          ...
          >
            Taste the
            <br />
            <span className="gradient-text-warm italic">Difference</span>
          </h3>
        </motion.div>
      </div>
    </section>
